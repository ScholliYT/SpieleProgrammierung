...
public class NetworkConfigManager {
	/**
	 * reads a xml config file from path and creates a NetworkConfig object
	 * containing the data.
	 */
...
//            marshaller.marshal(testconfig, System.out);
			...
			JOptionPane.showMessageDialog(null, "Failed to read config file at: " + path, "Failed to read config file",
					JOptionPane.ERROR_MESSAGE);
			System.out.println("Failed to read config file at: " + path);
			e.printStackTrace();
		} catch (IOException e) {
			JOptionPane.showMessageDialog(null, "Failed to create config file at: " + path,
					"Failed to create config file", JOptionPane.ERROR_MESSAGE);
