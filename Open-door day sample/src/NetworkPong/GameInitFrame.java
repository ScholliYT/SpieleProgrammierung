...
public class GameInitFrame extends JFrame{
	...
	public GameInitFrame() throws Exception{
		...
		panelClient.setBorder(new TitledBorder(null, "Einem Spiel beitreten", TitledBorder.LEADING, TitledBorder.TOP, null, null));
//		getContentPane().add(panelClient, BorderLayout.CENTER);
		...
		spinnerPortClient.setModel(new SpinnerNumberModel(25566, 0, 65535, 1));
		spinnerPortClient.setBounds(84, 43, 58, 20);
		spinnerPortClient.addChangeListener(change -> {
			this.selectedPort = (int) spinnerPortClient.getValue();
		});
		...
		panelOperationMode.setBorder(new TitledBorder(null, "M\u00F6chten Sie ein neues Spiel hosten oder beitreten?", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		lblPort.setBounds(10, 46, 92, 14);
		panelHostoptions.add(lblPort);
		
		spinnerPortHost = new JSpinner();
		spinnerPortHost.setModel(new SpinnerNumberModel(25566, 0, 65535, 1));
		spinnerPortHost.setBounds(112, 43, 62, 20);
		spinnerPortHost.addChangeListener(change -> {
			this.selectedPort = (int) spinnerPortHost.getValue();
		});
