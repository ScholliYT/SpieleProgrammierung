		...
		if (newBallX < BAT_WALL_OFFSET + bat.getImage().getWidth() / 2
			...
			double intersectY = ball.getActualY()
					- ((ball.getActualX() - (BAT_WALL_OFFSET + bat.getImage().getWidth() / 2.0))
							* (ball.getActualY() - newBallY)) / (ball.getActualX() - newBallX); // use slope and length
																								// to calculate exact
			...
			// TODO: take care of intersect with non center part of the ball (i.e. consider
			// ballradius) .... WIP
			if (intersectY + ballRadius >= upperBorderPaddle && intersectY - ballRadius <= lowerBorderPaddle) { // collision
																												// happened
																												// with
																												// the
																												// paddle
				double relativeIntersectY = bat.getY() - intersectY; // distance of intersection from center
				double normalizedIntersectY = relativeIntersectY / (bat.getImage().getHeight() / 2.0); // normalize
																										// relativeIntersectY
																										// (careful:
																										// relativeIntersectY
																										// could be be
																										// larger
																										// (bat.getImage().getHeight()
																										// / 2) in the
																										// case of a
																										// outer texture
																										// of the ball
				...
				double remainingTravelPercentage = Math
						...
						/ Math.sqrt(
								Math.pow(ball.getActualX() - newBallX, 2) + Math.pow(ball.getActualY() - newBallY, 2)); // calculate
																														// remaining
																														// travel
																														// distance
																														// with
